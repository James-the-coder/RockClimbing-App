...
const Filters = ({navigation}) => {
    ...
    let filterSearch= () => {
        ...
        db.transaction((tx) => {
                    if (filter==="Location"){
                        //location filters
                        query = 'SELECT crag, name from ascent INNER JOIN grade on grade.id=ascent.grade_id where sector<>"" and country="GBR" and crag LIKE ? and grade.fra_routes BETWEEN ? and ?'
                        parameters=[`%${info}%`, grades[gradeRange1-1],grades[gradeRange2-1]]
                    }
                    if (filter==="Name"){
                        //name filters
                        query = 'SELECT MAX(crag) AS crag, name from ascent INNER JOIN grade on grade.id=ascent.grade_id where sector<>"" and country="GBR" and name LIKE ? and grade.fra_routes BETWEEN ? and ?'
                        parameters=[`%${info}%`, grades[gradeRange1-1],grades[gradeRange2-1]]
                    }
                    if (filter===""){
                        //no filters
                        query = 'SELECT Distinct crag from ascent INNER JOIN grade on grade.id=ascent.grade_id WHERE sector<>"" AND country="GBR" and grade.fra_routes BETWEEN ? and ? '
                        parameters=[grades[gradeRange1-1],grades[gradeRange2-1]]
                    }
