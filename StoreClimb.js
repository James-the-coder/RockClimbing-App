...
const StoreClimb = ({navigation, route}) => {
  ...
  let SetData = async() => {
    ...
    if (tries === "" || isNaN(tries)){
        alert("invalid tries input")
        return;
    }
  ...
  let addData = () => {
    ...
    if (selected !== "" && date !== ""){
        cragIdArray.push({ date : date, id: selected.selected[0],name:selected.selected[1], liked: isSelected, crag: selected.selected[2], attempts: tries, climbGrade: grade})
    }
